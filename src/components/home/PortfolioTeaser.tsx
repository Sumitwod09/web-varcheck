          ...
          {PROJECTS.map((project, i) => (
            ...
            >
              <Link to={`/work/${project.slug}`} className="card-light block h-full group">
                ...
                <p className="mt-2 font-body text-sm text-varcheck-light-muted leading-relaxed line-clamp-2">
                  {project.description}
                </p>
                <div className="mt-4 flex flex-wrap gap-2">
                  {project.tech.slice(0, 3).map((t) => (
                    <span key={t} className="font-mono text-xs text-varcheck-accent bg-varcheck-accent/10 px-2 py-1 rounded">
                      {t}
                    </span>
                  ))}
                </div>
